...
`the other reference host`, whose layer order is `theme, base, mui, components, utilities`,
the package's zero-specificity `:where()` rules beat every Tailwind utility —
...
the strongest — the opposite of the intent. Hosts with no layers at all
(`the adopting host`) need no change: everything they write is unlayered and
therefore already wins.
